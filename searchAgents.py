...
class CornersProblem(search.SearchProblem):
    ...
    """
    # Data structure of a state for the corners problem:
    #     (position, corners)
#   where position is an (x,y) tuple coord in board
#   and corners is a binary integer representing corners visited

    def __getCorners(self, new_position, old_corners):
        """ Calling object is the old position. Input corod is the position of the
        new state. Compute the corners vector for the new state """
    ...
    def __getCrnVec(self, state):
        """ Returns corner vector"""
        return state[1]

    def isVisited(self, crn_num, state):
        """Returns true if the state has seen the corner, false o/w"""
    ...
    def getRmnCrns(self, state):
        """ Returns a list of the corners we have left to visit."""
# may do something
    ...
    def __init__(self, startingGameState):
        ...
        "*** YOUR CODE HERE ***"
        #   Implementation: states are tuples of gamestates and an integer (think
        #   binary)

        self.startState = (self.startingPosition,
                           self.__getCorners(self.startingPosition, 0))
...
def cornersHeuristic(state, problem):
    ...
    def getPathCost(point_seq):
        """Returns the cost of travelling along the path,
...
def foodHeuristic(state, problem):
    ...
    closestDistance = 0
    lengthList = []
    problem.startState = state[0]
    for foodCord in foodGrid.asList():
        xy1 = foodCord
        xy2 = position
        distance = mazeDistance(xy2, xy1, problem.startingGameState)
...
#    def getFoodInNeighborhood(self, state, size):
#        pac = self.startState.getPacmanPosition()
#        foodList = []
#        walls = state.getWalls()
#        width = walls.width
#        height = walls.height
#        for xD in range(min(size, width)):
#            for yD in range(min(size, height)):
#                if self.food[pac[0] + xD][pac[1] + yD]:
#                    FoodTuple = (pac[0] + xD, pac[1] + yD)
#                    foodList.append(FoodTuple)
#                elif self.food[pac[0] - xD][pac[1] + yD]:
#                    FoodTuple = (pac[0] - xD, pac[1] + yD)
#                    foodList.append(FoodTuple)
#                elif self.food[pac[0] - xD][pac[1] - yD]:
#                    FoodTuple = (pac[0] - xD, pac[1]- yD)
...
def heuristicFood(startState, problem):
    food = problem.food
    width = food.width
    print width
    height = food.height
    ...
    for i in range(width):
        for j in range(height):
            if k == 4:
                break
            if food[i][j]:
                 FoodTuples.append((i, j))
            k += 1
    print FoodTuples
    for v in FoodTuples:
        dist = mazeDistance(startState, v, problem.gameState)
